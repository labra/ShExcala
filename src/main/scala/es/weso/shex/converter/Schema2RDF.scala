...
object Schema2RDF extends Logging {
  ...
  def schema2RDF(schema: Schema, rdf: RDFBuilder): RDFBuilder = {
    ...
    shexSchema2RDF(schema.shexSchema, rdf)
    rdf
  }

  private def shexSchema2RDF(shexSchema: ShExSchema, rdf: RDFBuilder): RDFBuilder = {
    ...
    rules2RDF(shexSchema.shapes, schemaNode, rdf)
    start2RDF(shexSchema.start, schemaNode, rdf)
  }
