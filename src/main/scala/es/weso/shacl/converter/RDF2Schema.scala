...
import es.weso.shacl.{ Datatype, EmptyShape, GroupShape, Label }
import es.weso.shacl.{ RangeCardinality, SHACLSchema, Schema, Shape, ShapeConstr, ShapeExpr, SingleShape, TripleConstraint, UnboundedCardinalityFrom, ValueClass }
import es.weso.shacl.{ ValueConstr, ValueIRI, ValueLiteral, ValueObject, ValueSet }
import es.weso.shacl.Cardinality
import es.weso.shacl.Cardinality.defaultCardinality
import es.weso.shacl.Label.mkLabel
import es.weso.shacl.NodeKind
import es.weso.shacl.PREFIXES.{ sh_ClosedShape, sh_Shape, sh_ShapeClass, sh_datatype, sh_in, sh_maxCount, sh_minCount, sh_nodeKind, sh_predicate, sh_property, sh_valueShape }
import es.weso.shacl.ValueClass.nodeKindfromIRI
import es.weso.shacl.XSFacet.emptyFacets
...
object RDF2Schema
  ...
  private def shaclSchema(rdf: RDFReader): Try[SHACLSchema] = {
