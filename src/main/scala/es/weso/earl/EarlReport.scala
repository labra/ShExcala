...
case class EarlReport(
    items: List[SingleTestReport]
    ) {

  def addTestReport(r: SingleTestReport): EarlReport = {
    EarlReport(r :: items)
  }
  ...
  def addTestReport(
    ...
    EarlReport(SingleTestReport(passed, name, uriTest, testType, msg) :: items)
  }

  def concat(other: EarlReport): EarlReport = {
    EarlReport(items ++ other.items)
  }
  ...
  def generateEARL: Model = {
    ...
    model.add(wesin, doap_name, "ShExcala")
    ...
    model.add(wesin, dc_title, "ShExcala")
    ...
    model.add(release, doap_name, "ShExcala")
    model.add(release, doap_created,
    model.createTypedLiteral("2015-06-09", XSDDatatype.XSDdate))
...
object EarlReport {
  def initial = EarlReport(List())

  def sortReport(
      r1: SingleTestReport, 
      r2: SingleTestReport): Boolean = {
    r1.name < r2.name
  }
}
