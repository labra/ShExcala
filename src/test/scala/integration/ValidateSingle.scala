    ...
    it("Should be valid single") {
      val strData =
        """|@prefix ex: <http://example.org/> .
           ...
           |ex:tom  foaf:name "Tomas" ;  ex:experience ex:senior ;  ex:assignedIssue ex:issue1 .
           |ex:mary foaf:givenName "Maria"; foaf:lastName "Smith" .
           |ex:client1 ex:clientNumber 1 .
           |ex:issue1  ex:reportedBy ex:mary ; ex:reproducedBy ex:john ; ex:reproducedBy ex:tom .""".stripMargin
      ...
      val strSchema =
        """|prefix ex: <http://example.org/>
           ...
           |                                        )
           |}
           |<ClientShape> { ex:clientNumber xsd:integer    }
           |
           |<ConfirmedIssueShape> {
